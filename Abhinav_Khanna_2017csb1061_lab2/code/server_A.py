...
def check_credentials(username,password):
	flag = 0
	with open('login_credentials_A.csv') as csv_file:
		csv_reader = csv.reader(csv_file)
		for row in csv_reader:
			if username == row[0]:
				if password == row[1]:
					print('SERVER_A:Login Succesful')
					return 'SERVER_A:Login Succesful'
					flag = 1	

		if flag == 0:
			print('SERVER_A:Incorrect username/password')
			return 'SERVER_A:Incorrect username/password'
...
def threaded_communication(conn,addr):
	...
	conn.send('Username {} received at server A'.format(username))
