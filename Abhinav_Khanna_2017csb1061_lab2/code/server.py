...
def wait_ack(s,conn,result):
	#do nothing essentially
	#wait for acknowldegemnt
	print('Retransmitting.......')
	received_string = conn.recv(102)

	print('Retransmitting string:' + received_string)
	print(len(received_string))
	
	s.sendall(received_string)
	ack = s.recv(1)
	print('ack received at server')
	conn.sendall(ack)
...
def verify_checksum(received_string):
	...
	if received_string[-8:-1] == 'The End':
		actual_received_string = received_string[:-8]
	else:
		actual_received_string = received_string

	
	for i in range(0,len(actual_received_string)-1):
		actual_checksum = (actual_checksum + ord(actual_received_string[i]))%94 + 33 

	received_check_sum = actual_received_string[-1]

	print('received_check_sum:' + str(ord(received_check_sum)))
	print('actual_checksum:' + str(actual_checksum))

	if str(ord(received_check_sum)) == str(actual_checksum):
...
def threaded_communication(conn,addr,HOST_A,PORT_A,HOST_B,PORT_B,HOST_C,PORT_C,HOST_D,PORT_D):
	...
	if username == 'EXIT' or password == 'EXIT':
		...
		socket_a.sendall(str(frame_size))
		socket_b.sendall(str(frame_size))
		socket_c.sendall(str(frame_size))
		...
		while True:
			...
			if len(received_string) > 0:
				received_seq_number = received_string[0]

			if received_string[-8:-1] == 'The End' or len(received_string) == 0:
				if 'SERVER_A:Login Succesful' in result:
					socket_a.sendall(received_string)
					# ack = socket_a.recv(2)
					conn.sendall(received_seq_number + '1')

				elif 'SERVER_B:Login Succesful' in result:
					socket_b.send(received_string)
					# ack = socket_b.recv(2)
					conn.sendall(received_seq_number + '1')	

				elif 'SERVER_C:Login Succesful' in result:
					socket_c.send(received_string)
					# ack = socket_c.recv(2)
					conn.sendall(received_seq_number + '1')
			...
			if verify_checksum(received_string):
				if 'SERVER_A:Login Succesful' in result:
					socket_a.sendall(received_string)
					# ack = socket_a.recv(2)
					conn.sendall(received_seq_number + '1')
				...
				print('checksum does not match')
				# continue
				conn.sendall(received_seq_number + '0')		
